...
if __name__ == '__main__':
    ...
    train_data, train_label, test_data, test_label = load_data(
        dataset, train_params['normalize_type'])
    ...
    with graph.as_default():
        input_images = tf.placeholder(tf.float32, [
            None, image_size[0], image_size[1], image_size[2]], name='input_images')
        true_labels = tf.placeholder(
            tf.float32, [None, label_num], name='labels')
        is_train = tf.placeholder(tf.bool, shape=[])
        learning_rate = tf.placeholder(
            tf.float32, shape=[], name='learning_rate')
        keep_prob = tf.placeholder(tf.float32, shape=[], name='keep_prob')
        # build model
        logits, prob = build_model(input_images, args.k, args.T, label_num,
                                   is_train, keep_prob, args.if_a, args.if_b, args.if_c)
        ###     loss and accuracy
        loss_cross_entropy = tf.reduce_mean(
            tf.nn.softmax_cross_entropy_with_logits(logits=logits, labels=true_labels))
        ...
        train_op = optimizer.minimize(
            loss_cross_entropy + l2_loss * weight_decay)
    ...
    with tf.Session(config=config, graph=graph) as sess:
        ...
        batches_data_test, batches_labels_test, batch_count_test = into_batch(
            test_data, test_label, batch_size, shuffle=False)
        ...
        for epoch in range(1, total_epoches + 1):
            if epoch == total_epoches / 2:
                lr = lr * 0.1
            if epoch == total_epoches * 3 / 4:
                lr = lr * 0.1

            batches_data, batches_labels, batch_count = into_batch(
                train_data, train_label, batch_size, shuffle=True)
            ...
            for batch_id in range(batch_count):
                ...
                if (batch_id + 1) % 100 == 0:
                    print('epoch:', epoch, 'batch:',
                          batch_id + 1, 'in', batch_count)
                    print('loss:', result_per_bat[1],
                          'accuracy:', result_per_bat[2])

            saver.save(sess, os.path.join(
                result_dir, dataset + '_epoch_%d.ckpt' % epoch))
        ...
        np.save(os.path.join(result_dir, result_dir +
                             '_loss_train.npy'), np.array(loss_train))
        np.save(os.path.join(result_dir, result_dir +
                             '_acc_train.npy'), np.array(acc_train))
        np.save(os.path.join(result_dir, result_dir +
                             '_loss_test.npy'), np.array(loss_test))
        np.save(os.path.join(result_dir, result_dir +
                             '_acc_test.npy'), np.array(acc_test))
