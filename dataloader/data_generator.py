...
def download(data_type):
    ...
    if os.path.exists(data_path) == False:
        ...
        urllib.request.urlretrieve(
            url=url, filename=file_path, reporthook=download_progress)
...
def data_normalization(train_data_raw, test_data_raw, normalize_type):
    if normalize_type == 'divide-255':
        ...
        for channel in range(train_data_raw.shape[-1]):
            ...
            train_data[:, :, :, channel] = (
                train_data_raw[:, :, :, channel]-channel_mean)/channel_std
            test_data[:, :, :, channel] = (
                test_data_raw[:, :, :, channel]-channel_mean)/channel_std
...
def load_data(data_type, normalize_type):  # cifar-10  or cifar-100
    if data_type == 'svhn':
        ...
        train_data_raw, train_labels, test_data_raw, test_labels = svhn_preprocess(
            data_path_train, data_path_extra, data_path_test)
    else:
        data_path = download(data_type)
        train_data_raw, train_labels, test_data_raw, test_labels = cifar_preprocess(
            data_type, data_path)

    train_data, test_data = data_normalization(
        train_data_raw, test_data_raw, normalize_type)
